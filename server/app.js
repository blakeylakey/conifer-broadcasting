...
io.on('connection', (socket) => {
  ...
  socket.on('receiver join', (room) => {
    ...
    // Else continue
      ...
      socket.to(io.sockets.adapter.rooms[roomId].initiator_id).emit('new peer', socket.id);
    }
  });
  ...
  socket.on('new peer receiver sdp created', (sdp, id) => {
    ...
    io.to(io.sockets.adapter.rooms[roomId].initiator_id).emit('new peer receiver sdp', sdp, id);
  });
